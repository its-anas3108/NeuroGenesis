...
extracts per-ROI morphometric features, constructs a subject-specific brain
graph, and applies the proposed NeuroProp-X framework to estimate
stage-relevant regional vulnerability, fuse anatomical priors with learned
graph attention, and derive adaptive propagation representations. The
resulting enriched disease graph is processed by an edge-gated GATv2 model for
CN/MCI/AD classification. A Stage-Temporal Graph Transformer learns the
ordered CN->MCI->AD representation to estimate model-derived stage-transition
propensity. Explainable AI, ROI ranking, statistical analysis, and ablation
...
Module map (numbers follow the research design document):
    ...
    m01_dataset             OASIS-1/ADNI cohort, label mapping, subject splits
    ...
    m06_spatial_encoder     ROI patch dataset (shared cache/loading infra)
